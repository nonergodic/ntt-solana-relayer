...
pub struct RequestRelay<'info> {
  ...
  #[account(mut, address = instance.fee_recipient @ NttRelayerError::InvalidFeeRecipient)]
  ...
  #[account(
    ...
    constraint = registered_chain.base_price != u64::MAX @ NttRelayerError::RelayingToChainDisabled
  )]
...
pub fn request_relay(ctx: Context<RequestRelay>, args: RequestRelayArgs) -> Result<()> {
  ...
  require_gt!(args.gas_dropoff,
    ...
    NttRelayerError::ExceedsMaxGasDropoff
  );

  let total_fee_in_lamports = {
  ...
  //TODO should we (saturatively) subtract the rent from the total_fee?
  require_gte!(args.max_fee, total_fee_in_lamports, NttRelayerError::ExceedsUserMaxFee);
